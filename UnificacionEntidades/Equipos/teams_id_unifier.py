...
def team_id_unifier():
    ...
    sofifa = pd.read_csv(BASE_DIR / "UnificacionEntidades" / "Equipos" / "teams_16_20_sofifa.csv")
    fbdb = pd.read_csv(BASE_DIR / "UnificacionEntidades" / "Equipos" / "teams_16_20_fbdb.csv")
    
    # Cargar datos de WikiData
    wikidata_df = build_wikidata_team_mapping(BASE_DIR /  "UnificacionEntidades" / "Equipos" / "competiciones_wikidata.csv")
    
    definite, candidates, total = unify_entities(sofifa, fbdb, 'name', 'name', 'id', 'teamID', 
                                          threshold=55, threshold_candidates=40,
                                          output_file=BASE_DIR /  "UnificacionEntidades" / "Equipos" / "equipos_unificados.csv",
                                          output_file_candidates=BASE_DIR / "UnificacionEntidades" / "Equipos" / "equipos_candidatos.csv",
